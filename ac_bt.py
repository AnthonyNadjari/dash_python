...
def extract_data(start_date,end_date,tickers):
    ...
    data = pd.read_csv("/Users/anthonadj/PycharmProjects/dash_cours/HistoPrices/all_close.csv",index_col=0, sep=",",header=0,skiprows=0)
    data=data[tickers]

    data.index = pd.to_datetime(data.index,dayfirst=True)
    start_date = pd.to_datetime(start_date,dayfirst=True)
    end_date = pd.to_datetime(end_date,dayfirst=True)
    new_dates  = nearest_neighbors_sorted(np.array([start_date,end_date]),data.index)
    data["Dates"] = data.index

    data = pd.DataFrame(data.iloc[new_dates[0]:new_dates[1]+1]).set_index("Dates")
    return data
...
class long_leg:
    ...
    def payoff(self, stock_values,end_date):
        coupon = 0
        ac_coupon = 0
        ...
        elif len(first_ac)==0 and argmin_dates==0: #matu < today and no ac
            ac_coupon=0
            first_ac=[999]
        ...
        if not self.mem:
            coupon = (((stock_values["Values"].reset_index(drop=True)>(self.df["Coupon Barrier"].reset_index(drop=True)))[:last_coupon_barrier+1])*(self.df["Coupon"][:last_coupon_barrier+1])) .sum()
        elif self.mem:
...
def nearest_neighbors_sorted(x,y): #function used to offset the dates by a certain amount of months and take the nearest in the available dates
    ...
    y=pd.to_datetime(y,dayfirst=True)
    ...
    return np.sort(nearest_neighbor)
...
def backtest_global (df_param,freq,tickers,maturity,start_date,end_date,memory,basket_type,pdi_barrier,pdi_leverage,pdi_strike):
    global_stocks = extract_data(start_date,end_date,tickers)
